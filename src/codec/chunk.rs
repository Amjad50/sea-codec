...
pub struct SeaChunk<'inp> {
    ...
    pub scale_factors: BitUnpacker<'inp>,
    pub vbr_residual_sizes: BitUnpacker<'inp>,
    pub residuals: BitUnpacker<'inp>,
}

impl<'inp> SeaChunk<'inp> {
    pub fn from_slice(
        encoded: &'inp [u8],
        file_header: &SeaFileHeader,
        ...
        let mut lms: Vec<SeaLMS> = Vec::with_capacity(file_header.channels as usize);
        ...
        let scale_factors = {
            ...
            BitUnpacker::new_const_bits(scale_factor_bits, packed_scale_factors, scale_factor_items)
        };

        let vbr_residual_sizes = if matches!(chunk_type, SeaChunkType::Vbr) {
            ...
            BitUnpacker::new_const_bits(2, packed_vbr_residual_sizes, scale_factor_items)
                .with_addition(residual_size as u8 - 1)
        } else {
            BitUnpacker::empty()
        };
        ...
        let residuals = {
            let packed_residuals_bytes = if matches!(chunk_type, SeaChunkType::Vbr) {
                let mut vbr_residual_sizes_iter = vbr_residual_sizes.iter();
                let mut residual_bits: u32 = vbr_residual_sizes_iter
                    .by_ref()
                    .take(vbr_residual_sizes.len() - file_header.channels as usize)
                    .map(u32::from)
                    .sum();
                ...
                for size in vbr_residual_sizes_iter {
            ...
            if matches!(chunk_type, SeaChunkType::Vbr) {
                ...
                for item in vbr_residual_sizes.iter().take(
                ...
                BitUnpacker::new_var_bits(
                    &bitlengths,
                    packed_residuals,
                    frames_in_this_chunk * file_header.channels as usize,
                )
            } else {
                BitUnpacker::new_const_bits(
                    residual_size as u8,
                    packed_residuals,
                    frames_in_this_chunk * file_header.channels as usize,
                )
